...
public class SellInfo extends JPanel implements ActionListener, MouseListener, FocusListener{
	...
	String updatesql = "select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype from SellInfo a, ProductInfo b where a.Pid = b.Pid and 1 = ?";
	...
	private void findsellrecord() {
		...
		if (gettype.getSelectedItem().equals("--所有产品--")) {
			...
			// 对SellInfo ProductInfo表进行联合查询
	 		...
	 		updatetable("select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype from SellInfo a, ProductInfo b where a.Pid = b.Pid and (a.Pid like '%' ? '%' or b.PName like '%' ? '%')", tiaojianparas);
			...
			updatetable("select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype " +
	...
	private void look_for_year() {
		...
		if (gettype.equals("--所有产品--")) {
	 		...
	 		updatetable("select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype from SellInfo a, ProductInfo b where a.Pid = b.Pid and DATE_FORMAT(a.Outdate, '%Y') = ? ", look_more);
	...
	private void look_more_sellrecord() {
		...
		if (gettype.equals("--所有产品--")) {
			...
			// 不带产品类别的更新
	 		...
	 		updatetable("select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype from SellInfo a, ProductInfo b where a.Pid = b.Pid and DATE_FORMAT(a.Outdate, '%Y') = ? and DATE_FORMAT(a.Outdate, '%M') = ?", look_more);
		...
		if (e.getSource() == this.gettype) {
			...
			if (getIdorName.getText().trim().isEmpty()) {
				...
				if (gettype.getSelectedItem().equals("--所有产品--")) {
					...
					String sql = "select a.Pid, b.PName, b.Price, a.Num, (b.Price*a.Num), DATE_FORMAT(a.Outdate,'%Y-%m-%d %T'), b.Ptype from SellInfo a, ProductInfo b where a.Pid = b.Pid and b.Ptype = ?";
